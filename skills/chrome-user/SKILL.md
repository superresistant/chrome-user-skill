...
TargetIds and windowIds change every restart. Re-run discovery, never persist. Discovery exits 1 when only one window exists — only then bootstrap with `cdp open about:blank --window`, which Vivaldi may raise despite CDP `focus:false`, then re-run. Prefix misses auto-refresh the page cache, so tabs created outside `cdp open` still resolve
